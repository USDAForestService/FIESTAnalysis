...
anMODcompare <- function(bnd = NULL, 
  ...
  if (forprop) {
    ...
    SAestvars <- SAestvars[SAestvars %in% names(SAarea.multest)]
    ...
    MAarea.predselect.greg <- rbindlist(lapply(MAareaGREG, function(x) 
  ...
  for (j in 1:length(estvarlst)) {
    ...
    for (k in 1:length(tfilterlst)) {
      ...
      SAestvars <- SAestvars[SAestvars %in% names(SAtree.multest)]
