...
pcheck.xlsx <- function(wbnm, savewb=TRUE, outfn=NULL, outfolder=NULL,
  ...
  if (is.null(wbnm)) {
    ...
    if (!is.null(outfolder)) {
      wbnm <- file.path(outfolder, basename(wbnm))
    }

    if (!file.exists(wbnm)) {
     
      ## get basename and outfolder
      wb.basenm <- basename.NoExt(wbnm)
      ...
      if (outfn.date) {
        wbnm <- paste0(wb.basenm, "_", format(Sys.time(), "%Y%m%d"), ".xlsx")
      } else {
        wbnm <- paste0(wbnm, ".xlsx")
      }
      
      wbnm <- file.path(outfolder, wbnm)
      if (file.exists(wbnm)) {
...
setCells <- function(datsheet, estgrp, psegrp, nbrgrps=1, startrow=1, endrow=NULL,
  ...
  if (!allin1) {
    ...
    if (addSEcol) {
      ...
      psegrp[, tabcols] <- lapply(psegrp[, tabcols], check.numeric)
