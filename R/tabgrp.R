...
tabgrp <- function(GBpopdat = NULL,
  ...
  formallst <- names(formals(tabgrp)) 
  ...
  if (!is.null(GBpopdat)) {
    ...
    ## check tree table
    treex <- pcheck.table(GBpopdat$treex, caption="Tree table?", gui=FALSE)
    if (esttype == "TREE" && is.null(treex)) {
...
#      }
  ...
  ## Compile titlelst
  titlelst <- list(title.esttab=esttab.title, title.rowvar=title.rowvar, 
				title.colvar=title.colvar)
  if (rowgrp) titlelst$title.rowgrp <- title.rowgrp

  ## Compile returnlst
  returnlst <- list(esttab=esttabcol) 

  if (!allin1) {
    returnlst$psetab <- psetabcol
    titlelst$title.psetab <- psetab.title
  }
  returnlst$titlelst <- titlelst
  returnlst$cnames <- cnamesdf
