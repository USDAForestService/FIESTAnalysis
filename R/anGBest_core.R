#' Generate core tables using Green-Book module.
#'
#' Generates a set of core tables for a state.
#'
...
#' @param fill Logical. If TRUE, color fill is applied to all tables.
#' @param addSEcol Logical. If TRUE, percent standard error values are added to
#' columns side-by-side.
#' @param allin1 Logical. If TRUE, percent standard error values are added to
...
#' @param estround Integer. Number of digits after decimal to round estimate to.
#' @param pseround Integer. Number of digits after decimal to round percent standard error to.
#' @param divideby.vol String. divideby for volume tables ('hundred',
#' 'thousand', 'million').
#' @param datCore R list object. FIA data from database, returned from
#' FIESTAnalysis::anGBest_core_data. Includes datPlots, datStrata, ref_countycd, 
#' and ref_unitcd.
#' @author Tracey S. Frescino
#' @keywords data
#' @examples
#'
#' # Run a set of core tables for Utah, for the most current FIA Evaluation. 
#' # Note: this may take a few minutes
#' 
#' # anGBest_core("Utah", evalCur=TRUE)
#' 
#'
#' @export
anGBest_core <- function(state = NULL, 
                         datsource = "datamart",
                         data_dsn = NULL, 
                         evalCur = TRUE, 
                         evalEndyr = NULL, 
  ...
  if (is.null(datCore)) {
    datCore <- anGBest_core_data(state = state,
                           datsource = datsource,
                           data_dsn = data_dsn, 
                           evalCur = evalCur, 
                           evalEndyr = evalEndyr, 
    ...
    if (is.null(state)) {
      state <- datCore$datPlots$states
      if (length(state) != 1) {
        stop("invalid state... only one state allowed")
      }
    }
  }
  ...
  cond <- datPlots$tabs$cond
  tree <- datPlots$tabs$tree
  vspspp <- datPlots$tabs$vsubpspp
  vspstr <- datPlots$tabs$vsubpstr
  ...
  st <- pcheck.states(stcd, statereturn="ABBR")
  state <- pcheck.states(stcd, statereturn="MEANING")
  ...
  ## Check addSEcol
  ########################################################
  addSEcol <- pcheck.logical(addSEcol, varnm="addSEcol",
  ...
  ## Diameter classes
  #############################################################
  ref_diacl2in <- FIESTAutils::ref_diacl2in
  if (!"DIACL2IN" %in% names(tree)) {
    DIALUT <- ref_diacl2in[ref_diacl2in$MIN <= 31, ]
    DIALUT[DIALUT$MIN == 31, "MAX"] <- 300
    DIALUT[DIALUT$MIN == 31, "DIACL2IN"] <- "31+"
    DIALUT$DIACL2INCD <- seq(1:nrow(DIALUT))
    DIALUT2 <- DIALUT[DIALUT$MIN >= 5,]

    ## Append diameter classes to tree table
    datlut <- datLUTclass(x=tree, xvar="DIA", LUT=DIALUT, LUTclassnm="DIACL2IN",
		vars2keep="DIACL2INCD")
    tree <- datlut$xLUT

    datlut <- datLUTclass(x=tree03, xvar="DIA", LUT=DIALUT, LUTclassnm="DIACL2IN",
		vars2keep="DIACL2INCD")
    tree03 <- datlut$xLUT
  ...
  if ("SPGRPCD" %in% names(tree)) {
    ...
    tree <- datLUTnm(x=tree, xvar="SPGRPCD", FIAname=TRUE, group=TRUE)$xLUT
    ...
    tree03 <- datLUTnm(x=tree03, xvar="SPGRPCD", FIAname=TRUE, group=TRUE)$xLUT
  ...
  for (res in rescds) {
    ...
    pcfilter <- paste("RESERVCD ==", res)
    est <- modGBarea(GBpopdat=popdat00, 
                    landarea="FOREST", pcfilter=pcfilter,
                    rowvar="TIMBERCD.PROD", adj="none", sumunits=TRUE,
  ...
  est2 <- modGBarea(GBpopdat=popdat00, 
                    landarea="FOREST", pcfilter=cond2.filter,
                    adj="none", sumunits=TRUE,
                    table_opts=list(estround=estround, pseround=pseround)
                    )$est
  ...
  pcfilter <- "COND_STATUS_CD %in% c(3,4)"
  est <- modGBarea(GBpopdat=popdat00, 
                    landarea="ALL", pcfilter=pcfilter,
                    adj="none", sumunits=TRUE,
  ...
  pcfilter <- "COND_STATUS_CD %in% c(2,3,4)"
  est <- modGBarea(GBpopdat=popdat00, 
                    landarea="ALL", pcfilter=pcfilter,
                    adj="none", sumunits=TRUE,
                    table_opts=list(estround=estround, pseround=pseround)
  ...
  ## Nonsampled land
  ################################################################
  pcfilter <- "COND_STATUS_CD == 5"
  est <- modGBarea(GBpopdat=popdat00, 
                    landarea="ALL", pcfilter=pcfilter,
                    adj="none", sumunits=TRUE,
                    rowvar="COND_NONSAMPLE_REASN_CD", 
                    table_opts=list(row.FIAname=TRUE, 
                                    estround=estround, pseround=pseround)
                    )$est
...
#    footnote4 <- footnote
#  }
  estdat2 <- tabgrp(GBpopdat=popdat, esttype="AREA", 
                    landarea=landarea, sumunits=TRUE, 
                    rowvar=rowvar, colvar=colvar, 
                    colgrp=colgrp, colgrpcd=colgrpcd,
                    rowgrp.subtot=rowgrp.subtot,  
                    title.rowgrp=title.rowgrp,
                    table_opts = list(row.FIAname=row.FIAname, 
                                      ...
                                      estround=estround, pseround=pseround,
                                      divideby=divideby),
                    title_opts = list(title.colvar=title.colvar)
                    )
  esttab <- estdat2$esttab
  tabtitle <- estdat2$titlelst$title.esttab
  title.rowvar <- estdat2$titlelst$title.rowvar
  title.colvar <- estdat2$titlelst$title.colvar
  cnames <- estdat2$cnames
  title.rowgrp <- estdat2$titlelst$title.rgrp
  psetab <- estdat2$psetab
  psetab.title <- estdat2$titlelst$title.psetab

  write2xlsx(esttab=esttab, psetab=psetab, 
             tabtitle=paste0("Table ", tabnm, ". ", tabtitle), 
             title.colvar=NULL, title.rowvar=title.rowvar, 
             title.rowgrp=title.rowgrp, rowgrp=rowgrp, 
             rowgrp.subtot=rowgrp.subtot,
             colgrp=colgrp, cnames=cnames, coltottxt=coltottxt, 
             outfolder=outfolder, allin1=allin1, 
             addSEcol=addSEcol, fill=fill, 
             wbnm=wbnm, sheetnm=tabnm, 
             footnote1=footnote1, footnote2=footnote2, 
             footnote3=footnote3, cellwidth=cellwidth+3)
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             coltottxt=coltottxt, 
             outfolder=outfolder, 
             fill=fill, allin1=allin1, 
             addSEcol=addSEcol, cellwidth=cellwidth, 
             wbnm=wbnm, sheetnm=tabnm, 
...
#  }
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             tabtitle=paste0("Table ", tabnm, ". ", tabtitle), 
             title.rowvar=title.rowvar, title.colvar=NULL, 
             outfolder=outfolder, allin1=allin1, 
             rowgrp=FALSE, 
             colgrp=TRUE,
             cnames=cnames, coltotal=TRUE, coltottxt=coltottxt,            
             addSEcol=addSEcol, fill=fill, 
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             outfolder=outfolder, 
             fill=fill, 
             allin1=allin1, addSEcol=addSEcol, 
             cellwidth=cellwidth+1, 
             wbnm=wbnm, sheetnm=tabnm, 
             footnote1=footnote1, footnote2=footnote2, footnote3=footnote3)
...
print(paste("Table", tabnm))
  ...
  estdat2 <- tabgrp(GBpopdat=popdat, 
                    esttype="TREE", 
                    landarea=landarea, sumunits=TRUE, 
                    estvar=estvar, 
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             tabtitle=paste0("Table ", tabnm, ". ", tabtitle), 
             title.rowvar=title.rowvar, title.colvar=title.colvar, 
             rowgrp=rowgrp, title.rowgrp=title.rowgrp, 
             rowgrp.subtot=rowgrp.subtot,
             coltottxt=coltottxt, 
             outfolder=outfolder, 
             fill=fill, allin1=allin1, 
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             tabtitle=paste0("Table ", tabnm, ". ", tabtitle), 
             title.rowvar=title.rowvar, title.colvar=title.colvar, 
             rowgrp=rowgrp, rowgrp.subtot=rowgrp.subtot, 
             title.rowgrp=title.rowgrp, 
             coltottxt=coltottxt, 
             outfolder=outfolder, 
             fill=fill, allin1=allin1, 
...
#  }
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             outfolder=outfolder, 
             allin1=allin1, addSEcol=addSEcol, 
             fill=fill, cellwidth=cellwidth+2, 
             wbnm=wbnm, sheetnm=tabnm, 
             footnote1=footnote1, footnote2=footnote2, footnote3=footnote3)
...
#    footnote4 <- footnote
#  }
  estdat2 <- tabgrp(GBpopdat=popdat, esttype="TREE", 
                    landarea=landarea, sumunits=TRUE, 
                    estvar=estvar, estvar.filter=estvar.filter, 
                    rowvar=rowvar, colvar=colvar, 
                    colgrp=colgrp, colgrpcd=colgrpcd, 
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             colgrp=colgrp, cnames=cnames, coltottxt=coltottxt,
             outfolder=outfolder, allin1=allin1, 
             addSEcol=addSEcol, fill=fill, cellwidth=cellwidth+2, 
             wbnm=wbnm, sheetnm=tabnm, 
             footnote1=footnote1, footnote2=footnote2, footnote3=footnote3)
...
print(paste("Table", tabnm))
  ...
  estdat <- modGBtree(GBpopdat=popdat, 
                              landarea=landarea, sumunits=TRUE, 
                              estvar=estvar, estvar.filter=estvar.filter, 
                              rowvar=rowvar, colvar=colvar,
                              returntitle=TRUE, 
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             colgrp=colgrp, cnames=cnames, coltottxt=coltottxt, 
             outfolder=outfolder, 
             allin1=allin1, fill=fill, 
             addSEcol=addSEcol, cellwidth=cellwidth, 
             wbnm=wbnm, sheetnm=tabnm, 
             footnote1=footnote1, footnote2=footnote2, 
             footnote3=footnote3, footnote4=footnote4)
...
print(paste("Table", tabnm))
  ...
  estdat2 <- tabgrp(GBpopdat=popdat03, esttype="TREE", 
                    landarea=landarea, sumunits=TRUE, 
                    estvar=estvar, estvar.filter=estvar.filter, 
                    rowvar=rowvar, colvar=colvar, 
                    colgrp=colgrp, colgrpcd=colgrpcd, 
...
print(paste("Table", tabnm))
  ...
  estdat <- modGBtree(GBpopdat=popdat03, 
                              landarea=landarea, sumunits=TRUE, 
                              estvar=estvar, estvar.filter=estvar.filter, 
                              rowvar=rowvar, colvar=colvar, 
                              returntitle=TRUE, 
...
print(paste("Table", tabnm))
  ...
  estdat2 <- tabgrp(GBpopdat=popdat, esttype="TREE", 
                    sumunits=TRUE, landarea=landarea, 
                    estvar=estvar, estvar.filter=estvar.filter, 
                    rowvar=rowvar, colvar=colvar, 
                    rowgrp.subtot=rowgrp.subtot, 
                    table_opts=list(row.FIAname=row.FIAname, 
...
print(paste("Table", tabnm))
  ...
  title.colvar <- estdat2$titlelst$title.colvar
  cnames <- estdat2$cnames
  title.rowgrp <- estdat2$titlelst$title.rowgrp
  psetab <- estdat2$psetab
  psetab.title <- estdat2$titlelst$title.psetab
...
print(paste("Table", tabnm))
  ...
  estdat2 <- tabgrp(GBpopdat=popdat, esttype="AREA", 
                    ...
                    rowgrp.subtot=rowgrp.subtot, 
                    title.rowgrp=title.rowgrp,
                    table_opts=list(col.FIAname=col.FIAname,
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             title.rowvar=title.rowvar, title.colvar=NULL, 
             title.rowgrp=title.rowgrp, rowgrp=rowgrp, rowgrp.subtot=rowgrp.subtot,
             colgrp=TRUE, cnames=cnames, coltotal=TRUE, coltottxt=coltottxt, 
             outfolder=outfolder, 
             allin1=allin1, fill=fill, 
             addSEcol=addSEcol, cellwidth=cellwidth, 
             wbnm=wbnm, sheetnm=tabnm, 
...
print(paste("Table", tabnm))
  ...
  write2xlsx(esttab=esttab, psetab=psetab, 
             ...
             title.rowvar=title.rowvar, title.colvar=NULL, 
             title.rowgrp=title.rowgrp, rowgrp=rowgrp, rowgrp.subtot=rowgrp.subtot,
             colgrp=colgrp, cnames=cnames, coltotal=TRUE, coltottxt=coltottxt, 
             outfolder=outfolder, 
             allin1=allin1, fill=fill, 
             addSEcol=addSEcol, cellwidth=cellwidth, 
             wbnm=wbnm, sheetnm=tabnm, 
...
print(paste("Table", tabnm))
  ...
  estdat1 <- tabgrp(GBpopdat=popdat, esttype="TREE", 
                    landarea=landarea, sumunits=TRUE,
                    estvar=estvar, estvar.filter=estvar.filter,
                    rowvar=rowvar, colvar=colvar, 
                    colgrp=colgrp, 
  ...
  tabtitle <- estdat1$titlelst$title.esttab
  title.rowvar <- estdat1$titlelst$title.rowvar
  title.colvar <- estdat1$titlelst$title.colvar
  cnames <- estdat1$cnames
  title.rowgrp <- estdat1$titlelst$title.rowgrp
  psetab <- estdat1$psetab
  psetab.title <- estdat1$titlelst$title.psetab


  estdat2 <- tabgrp(GBpopdat=popdat, esttype="TREE", 
                    landarea=landarea, sumunits=TRUE,
                    estvar=estvar2, estvar.filter=estvar.filter2,
                    rowvar=rowvar, colvar=colvar, 
                    colgrp=colgrp, 
                    rowgrp.subtot=rowgrp.subtot, 
                    table_opts=list(row.orderby=row.orderby,
