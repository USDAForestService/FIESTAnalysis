#' ANALYSIS - Generate core tables for Model-Assisted custom population.
#'
#' Generates a set of core tables based on area of interest.
#'
#' If variables are NULL, then it will prompt user to input variables.
#'
#' @param MApopdat R object. Population data from database, returned from
#' modMApop() function for a specified area of interest.
...
#' @keywords data
#' @export
anMApop_core <- function(MApopdat, 
                         MAmethod = "greg",
                         title.ref, 
                         xlsx = FALSE, 
                         outfolder = NULL, 
  ...
  formallst <- names(formals(anMApop_core)) 
...
#  MApopdat$pltcondx <- merge(MApopdat$pltcondx,
#		ref_codes[ref_codes$VARIABLE == "DSTRBCD", c("VALUE", "GROUPCD")],
#		by.x="DSTRBCD1", by.y="VALUE")
#  names(MApopdat$pltcondx)[names(MApopdat$pltcondx) == "GROUPCD"] <- "DSTRBGRPNM"

  ## Diameter class for DIA - DIACL
  if (is.null(treedia.brks)) {
    treedia.brks=c(0,5,10,15,20,25,50,100)
  }
  datlut <- datLUTclass(x=MApopdat$treex, xvar="DIA", cutbreaks=treedia.brks)
  MApopdat$treex <- datlut$xLUT
  ...
  estdat <- modMAarea(MApopdat=MApopdat, 
                      MAmethod=MAmethod, 
                      landarea=landarea, 
                      pcfilter="DSTRBCD1 > 0", 
                      rowvar=rowvar, 
  ...
  estdat <- modMAtree(MApopdat=MApopdat,
                      MAmethod=MAmethod, 
                      landarea=landarea,
                      estvar=estvar,
