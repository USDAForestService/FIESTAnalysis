...
write2xlsx <- function(esttab, 
  ...
  ###############################################################
  wbnm <- pcheck.xlsx(wbnm, outfn.date=outfn.date, outfolder=outfolder, 
			overwrite=overwrite)
  ...
  if (colgrp) {
    ...
    if (length(colgrpnames) < length(colStart.nbr:ctitle.endcol) && 
		length(colStart.nbr:ctitle.endcol) - length(colgrpnames) == 1) {
      stop("invalid number of cells in table... check column total")
    }
