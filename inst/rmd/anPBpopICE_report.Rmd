...
#  rawfolder <- "E:/workspace/FIESTA/FIESTA_PB/outfolder/estimates/rawdata"
#  AOInm <- "Caribbean"
#  yearT1 <- 2017
#  yearT2 <- 2018
#  #ice.QAQCfn <- "E:/workspace/ICE/prototype/_reports/gtac_ICE/states/Caribbean/Photo_data/StJohns_QAQC.csv"
#  ice.QAQCfn <- NULL
#  outfn.pre <- AOInm
...
if (is.null(ice.QAQCfn) || ice.QAQCfn == "") {
  ice.QAQC <- NULL
} else {
  if (!file.exists(ice.QAQCfn)) stop(ice.QAQCfn, " does not exist")
  ice.QAQC <- fread(ice.QAQCfn)
...
# -------------------------------------------------------------------------------------
summaryInfo = as.data.frame(matrix(nrow=1,ncol=22))
...
summaryInfo[1,]$forLuAmtChangeAnn = summaryInfo[1,]$forLuAmtChange / (T2 - T1)
summaryInfo[1,]$forLuAmtChangeAnnUnc = sqrt(summaryInfo[1,]$forLuAmtChangeUnc^2 / (T2 - T1)^2)
...
Over the analysis period (`r yearT1` to `r yearT2`), Forest LU in `r AOInm` `r summaryInfo[1,]$forLuDecrIncr` by `r paste0(abs(as.numeric(setDecPlaces(summaryInfo[1,]$forLuAmtChange,3))),"% (+/-",setDecPlaces(summaryInfo[1,]$forLuAmtChangeUnc,3),"%)")`, from `r paste0(setDecPlaces(summaryInfo[1,]$forLuEstT1,3),"%")` (+/-`r paste0(setDecPlaces(summaryInfo[1,]$forLuUncT1,3),"%")`) in `r yearT1` to `r paste0(setDecPlaces(summaryInfo[1,]$forLuEstT2,3),"%")` (+/-`r paste0(setDecPlaces(summaryInfo[1,]$forLuUncT2,3),"%")`) in `r yearT2`; 
this is an average annual change of `r paste0(abs(as.numeric(setDecPlaces(summaryInfo[1,]$forLuAmtChangeAnn,3))),"% (+/-",setDecPlaces(summaryInfo[1,]$forLuAmtChangeAnnUnc,3),"%)")`. `r forLuChgStmt1`

&nbsp; 
&nbsp;

Tree LC in `r AOInm` `r summaryInfo[1,]$treeLcDecrIncr` over the analysis period by `r paste0(abs(as.numeric(setDecPlaces(summaryInfo[1,]$treeLcAmtChange,3))),"% (+/-",setDecPlaces(summaryInfo[1,]$treeLcAmtChangeUnc,3),"%)")`, from `r paste0(setDecPlaces(summaryInfo[1,]$treeLcEstT1,3),"%")` (+/-`r paste0(setDecPlaces(summaryInfo[1,]$treeLcUncT1,3),"%")`) in `r yearT1` to `r paste0(setDecPlaces(summaryInfo[1,]$treeLcEstT2,3),"%")` (+/-`r paste0(setDecPlaces(summaryInfo[1,]$treeLcUncT2,3),"%")`) in `r yearT2`; 
this is an average annual change of `r paste0(abs(as.numeric(setDecPlaces(summaryInfo[1,]$treeLcAmtChangeAnn,3))),"% (+/-",setDecPlaces(summaryInfo[1,]$treeLcAmtChangeAnnUnc,3),"%)")`. `r treeLcChgStmt1`
...
################ %T2-LC within T2-LU for `r AOInm` (`r yearT1` to `r yearT2`)
&nbsp;
&nbsp;
```{r T2-LCover_LUse,echo=FALSE, fig.height=4.5, fig.width = 10, dpi=300}
